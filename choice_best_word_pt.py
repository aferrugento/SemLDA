	...
	for i in range(len(g)):
		if i % 16 == 0:
			...
			if int(g[i]) < 0:
				...
				max_lemma = maxCount(wn_pt.get_synset(int(g[i]))[0], aux)
				aux += 1
				pos = wn_pt.get_synset(int(g[i]))[1]
				#print max_lemma.name.lower() + '_' + pos
				a = words_new.get(str(max_lemma.lower()) + '_' + str(pos))
